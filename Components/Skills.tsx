    ...
    <section className="section" id="skills">
      <motion.div className="container mx-auto"  {...animationProps}>
        ...
        <div className="flex flex-wrap gap-[60px] justify-center mt-[50px]">
            {Icons.map((icon) => (
              <div key={icon.id} className="flex flex-col gap-y-4 items-center justify-center"><span className="text-[#FFFFFF]  hover:opacity-70  transition duration-1500 ease-in">{icon.icon}</span>
              <p className="text-[#FFFFFF]">{icon.name}</p></div>
            ))}
        </div>
      </motion.div>
    </section>
